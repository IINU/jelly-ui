...
type Props<T> = {
  ...
  rows: T[]
  /**
   * Ref to the scroll container (the element that overflows). Exposed so
   * consumers can observe or restore the table's scroll position.
   */
  scrollContainerRef?: Ref<HTMLDivElement>
}

export function Table<T>({ columns, rows, scrollContainerRef }: Props<T>) {
  return (
    <div ref={scrollContainerRef} className="jui-overflow-x-auto">
      <table className="jui-min-w-full jui-table-auto jui-border-collapse">
        ...
        <tbody>
          {rows.map((item, i) => (
            <tr
              key={`row-${i}`}
              className="jui-border-b jui-border-primary-100 jui-whitespace-nowrap"
            >
              {columns.map(({ row }, j) => (
                <td
                  key={
                    row.idExtractor
                      ? row.idExtractor(item, j)
                      : `row-col-${i}-${j}`
                  }
